...
export async function POST(request: Request) {
  try {
    ...
    if (result.success && result.assetIndex !== undefined) {
      // Ensure creator exists as a user
      await prisma.user.upsert({
        where: { address: input.creator },
        update: {},
        create: {
          address: input.creator,
          username: `user_${input.creator.slice(0, 8)}`,
        },
      });
      ...
      console.warn(`[register-asset] Asset registered but index not yet available from indexer. Sync will be required to see it in the DB.`);
    }
