...
import { Search, Filter, LayoutGrid, List, Heart } from "lucide-react";
import { featuredAssets } from "../../data/mockData";
import Link from "next/link";

export default function Watchlist() {
  const assets = [...featuredAssets];
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div>
          ...
          <p className="font-mono text-sm text-text-secondary mt-2">
            Track your favorite digital assets and collections.
          </p>
        </div>
      </div>
          ...
          <Link href={`/assets/${asset.id}`} key={i} className="block group">
            <Card className="p-0 overflow-hidden bg-obsidian border-white/5 hover:-translate-y-1 hover:shadow-[0_8px_30px_rgba(204,255,0,0.1)] hover:border-lime/30 transition-all duration-300">
              <div className="h-48 relative bg-black">
                ...
                <button className="absolute top-4 right-4 w-8 h-8 rounded-full bg-white/10 backdrop-blur-md border border-white/10 flex items-center justify-center text-red-500 hover:bg-white/20 transition-colors">
              ...
              <div className="p-5">
                ...
                <div className="mt-4 pt-4 border-t border-white/5 flex items-end justify-between font-mono">
                  <div>
                    <p className="text-[10px] text-text-muted uppercase tracking-widest mb-1">Current Price</p>
